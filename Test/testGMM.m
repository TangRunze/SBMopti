...
for iGraph = 1:1000
    ...
    fileID = fopen(['graph', num2str(iGraph)],'w');
    fprintf(fileID,[repmat('%d ',1,300),'\n'],adjMatrix);
    fclose(fileID);
    
    fileID = fopen(['xhat', num2str(iGraph)],'w');
    fprintf(fileID,[repmat('%d ',1,3),'\n'],xHat);
    fclose(fileID);
    ...
    fileID = fopen(['tauhat', num2str(iGraph)],'w');
    fprintf(fileID,[repmat('%d ',1,300),'\n'],tauHat);
    fclose(fileID);

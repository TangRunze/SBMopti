...
errorRateASGEVecMean = zeros(nR, 1);
errorRateASGEVecVar = zeros(nR, 1);
errorRateOptiVecMean = zeros(nR, 1);
errorRateOptiVecVar = zeros(nR, 1);
errorRateOptiOptiVecMean = zeros(nR, 1);
errorRateOptiOptiVecVar = zeros(nR, 1);
...
%         errorASGEGMMVec(iInd) = errorratecalculator(tauStar, tauHat0, nVertex, nBlock);
    ...
    errorRateASGEVecMean(iR) = mean(errorASGEVec);
    errorRateASGEVecVar(iR) = var(errorASGEVec);
    
    errorRateOptiVecMean(iR) = mean(errorOptiVec);
    errorRateOptiVecVar(iR) = var(errorOptiVec);
    
    errorRateOptiOptiVecMean(iR) = mean(errorOptiOracleVec);
    errorRateOptiOptiVecVar(iR) = var(errorOptiOracleVec);
end

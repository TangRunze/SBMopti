function [nVertex, adjMatrix, adjMatrixDA, ind, tauStar] = ...
    datareader_celegans(iGraph)
% Read the existing data.

rng shuffle;

if exist(['data/celegans-graph' int2str(iGraph) '.mat'], 'file') == 0
    
    disp(['Generating subgraph ' int2str(iGraph) '...'])
    
    % Read the entire graph
    edgelist = textread(['data/celegans_c_edgelist.txt']);
    bias = min(min(edgelist));
    edgelist = edgelist - bias + 1;
    ...
    % Read the labels
    tauStar0 = textread(['data/celegans_cl.txt'])';
    ...
    ind1 = randsample(n1, 50, true);
    ind2 = randsample(n2, 50, true) + n1;
    ind3 = randsample(n3, 50, true) + n1 + n2;
    ind = [ind1; ind2; ind3];
    ...
    % Save the data
    save(['data/celegans-graph' int2str(iGraph) '.mat'], 'adjMatrixDA', ...
        'ind', 'adjMatrix', 'nVertex', 'tauStar');
    
else
    % Read the existing data
    data = load(['data/celegans-graph' int2str(iGraph) '.mat']);
